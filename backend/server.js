  ...
  try {
    ...
    return stripe.customers.create(
      ...
      stripe.paymentIntents.create({
        amount: Math.floor((req.body.ftp * 100)),
        currency: 'inr',
...
// ----------------------vieworderbillingadmin---------------------------------
app.get("/vieworderbillingadmin12", (req, res) => {
  db.query("SELECT checkout.*, checkout.uid,userdetails.name,userdetails.email,userdetails.phone,userdetails.email FROM checkout JOIN userdetails ON checkout.uid = userdetails.user_id GROUP BY checkout.billno order by checkout.tdt desc"
  ,(err, result) => {
      if (err) {
        console.log(err);
      }
...
app.post("/sendmail", (req, res) => {
     ...
     transporter.sendMail({
      ...
      text: `Hi,${req.body.name} Due Date Reminder,
 Bill Number : ${req.body.billno}
 Due Date :${req.body.tdt}`
      // html: "<b>There is a new article. It's about sending emails, check it out!</b>",
    }).then(info => {
      res.send("success");
